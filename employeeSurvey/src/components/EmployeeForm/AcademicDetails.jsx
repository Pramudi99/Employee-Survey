...
const handleChange = (e, fieldName, examIndex = null, subjectIndex = null) => {
  ...
  if (examIndex !== null) {
    ...
    if (fieldName === "examType") {
      updatedExams[examIndex] = {
        ...
        examType: value,
        stream: "", // clear stream
        subjectTable: [{ subjectName: "", subjectResults: "" }] // reset subjects
      };
    ...
    if (fieldName === "examType" || fieldName === "stream") {
      updatedErrors.examResults[examIndex].subjectTable = [{ subjectName: false, subjectResults: false }];
    }
  ...
  const getAvailableOLSubjects = (examIndex) => {
    const selected = formData.examResults[examIndex].subjectTable.map(s => s.subjectName.trim());
    return OL_SUBJECTS.map(s => s.label).filter(label => !selected.includes(label));
  };
  ...
  const isLastExamValid = () => {
    const lastExam = formData.examResults[formData.examResults.length - 1];
    ...
    if (examType === "A/L" && !stream) return false;
    if (!indexNumber || !attemptYear || !attempt) return false;
  
    // Check all subjects
    for (let subj of subjectTable) {
      if (!subj.subjectName || !subj.subjectResults) return false;
    }
      ...
      <Grid item xs={11.7} container spacing={1} sx={{ ml: 1 }}>
                    ...
                    <React.Fragment key={subjectIndex}>
                      <Grid item xs={5}>
                        ...
                        {exam.examType === "A/L" && exam.stream ? (
                          <Autocomplete
                            freeSolo
                            options={getAvailableSubjects(examIndex)}
                            inputValue={subject.subjectName || ''}
                            onInputChange={(event, newInputValue) => {
                              const syntheticEvent = { target: { value: newInputValue } };
                              handleChange(syntheticEvent, "subjectName", examIndex, subjectIndex);
                            }}
                            renderInput={(params) => (
                              <TextField
                                {...params}
                          ...
                          <Autocomplete
                          freeSolo
                          options={getAvailableOLSubjects(examIndex)}
                          inputValue={subject.subjectName || ''}
                          onInputChange={(event, newInputValue) => {
                            const syntheticEvent = { target: { value: newInputValue } };
                            handleChange(syntheticEvent, "subjectName", examIndex, subjectIndex);
        ...
        {/* {!isLastExamValid() && (
          <FormHelperText error sx={{ mt: -1, ml: 1 }}>
            Complete the previous exam before adding a new one.
          </FormHelperText>
        )} */}
